...
namespace _3DChess
{
    public class Piece
    {
                ...
                case Type.Pawn:
                    ...
                    else // !IsWhite
                    {
                        ...
                        if (Position.X + 1 < 8 && Position.Y - 1 >= 0 && Board.board[(int)Position.X + 1, (int)Position.Y - 1, (int)Position.Z].PieceType != Type.Empty && Board.board[(int)Position.X + 1, (int)Position.Y - 1, (int)Position.Z].IsWhite != IsWhite)
                            possibleMoves.Add(new Vector3((int)Position.X + 1, (int)Position.Y - 1, (int)Position.Z));
                        if (Position.X - 1 < 8 && Position.X - 1 >= 0 && Position.Y - 1 >= 0 && Board.board[(int)Position.X - 1, (int)Position.Y - 1, (int)Position.Z].PieceType != Type.Empty && Board.board[(int)Position.X - 1, (int)Position.Y - 1, (int)Position.Z].IsWhite != IsWhite)
                            possibleMoves.Add(new Vector3((int)Position.X - 1, (int)Position.Y - 1, (int)Position.Z));
                    ...
                    for (int i = -2; i < 3; i++)
                        for (int j = -2; j < 3; j++)
                            for (int k = -2; k < 3; k++)
                            {
                                ...
                                if (Board.IsInBound(v) && (Board.board[(int)v.X, (int)v.Y, (int)v.Z].PieceType == Type.Empty || Board.board[(int)v.X, (int)v.Y, (int)v.Z].IsWhite != IsWhite) && Math.Abs(i) + Math.Abs(j) + Math.Abs(k) == 3 && (i == 0 || j == 0 || k == 0))
                                {
                                    possibleMoves.Add(v);
                                }
                            }
        ...
        private List<Vector3> GetPossibleCasesFromStartingPointAndDirection(Vector3 v, int dx, int dy, int dz)
        {
            ...
            else if (v.X + dx >= 0 && v.X + dx < 8 && v.Y + dy >= 0 && v.Y + dy < 8 && v.Z + dz >= 0 && v.Z + dz < 3 && Board.board[(int)v.X + dx, (int)v.Y + dy, (int)v.Z + dz].IsWhite != IsWhite)
                l.Add(new Vector3(v.X + dx, v.Y + dy, v.Z + dz));
            return l;
        }
    }
